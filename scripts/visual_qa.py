...
def process_images_and_text(image_path, query, client):
    logger.info(f'Starte process_images_and_text für {image_path}')
    messages = [
        {
            "role": "user",
            "content": [
...
class VisualQATool(Tool):
    ...
    def forward(self, image_path: str, question: Optional[str] = None) -> str:
        logger.info(f'Starte VisualQATool.forward für {image_path}')
        output = ""
        ...
        try:
            ...
            logger.error(f"Fehler in VisualQATool.forward: {e}")
            if "Payload Too Large" in str(e):
                new_image_path = resize_image(image_path)
                logger.info(f"Bild wurde verkleinert: {new_image_path}")
        ...
        logger.debug(f'VisualQATool.forward Ergebnis: {output[:200]}...')
        return output
